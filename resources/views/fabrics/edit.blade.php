                    ...
                    <form action="{{ route('fabrics.update', $product->id) }}" method="POST">
                        ...
                        @if(Auth::user()->role === 'staff')
                            <div class="alert alert-info mb-3">
                                <i class="bi bi-info-circle"></i> <strong>Staff:</strong> Anda hanya dapat menambah stok, tidak dapat langsung mengedit nilai stok. Stok akan berkurang otomatis saat ada penjualan.
                            </div>
                        @endif
                            ...
                            <div class="row g-3 mb-3 baris-warna">
                                ...
                                @else
                                    <div class="col-md-3">
                                        ...
                                        <input type="text" class="form-control bg-light" value="{{ $variant->stock }} pcs" readonly>
                                        <input type="hidden" name="current_stocks[]" value="{{ $variant->stock }}">
                                    </div>
                                    <div class="col-md-3">
                                        <label class="form-label">Tambah Stok</label>
                                        <input type="number" name="add_stocks[]" class="form-control" value="0" min="0" placeholder="+0">
                                        <small class="text-muted">Masukkan jumlah yang ingin ditambah</small>
                                    </div>
                                    <div class="col-md-2 d-flex align-items-end">
                                        <button type="button" class="btn btn-outline-danger w-100 btn-hapus" {{ $loop->first ? 'style=display:none;' : '' }}>Hapus</button>
                                    </div>
